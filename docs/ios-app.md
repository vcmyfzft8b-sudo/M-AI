...
- Latest ready billing-enabled preview: `https://memo-74r6hyzn7-nace-valencics-projects.vercel.app`, deployment `dpl_DghKCxdmuxm8TXSo37jN14kK18zA`, commit `0110923b`. Native loading-screen changes are in the local binary; this server deployment remains compatible. TLS-verified staging checks confirmed the entitlement ledger and both account-lifecycle tables exist with RLS enabled.
- Actual synthetic password login, AI-processing consent and all 23 onboarding steps passed in the iPhone simulator, including the demonstration quiz, practice answer and tutor controls. The real StoreKit paywall displayed three-day trials and opened Apple’s purchase sign-in dialog. No purchase has completed: a Sandbox tester is still needed, and the App Store Connect browser session expired before tester setup. These onboarding demonstrations do not verify generated study content or the purchase lifecycle.
...
- The actual paywall exposed vertically compressed purchase/restore actions. Set their flex sizing and minimum height explicitly; changed login legal copy from the disabled-text token to the normal muted-text token. The full web build passed (`ios/build/paywall-layout-build.log`); refreshed Preview visual verification is pending.
