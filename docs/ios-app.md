...
- Created and downloaded the **Memo Billing Server** In-App Purchase key with the account holder's explicit confirmation. Its private key is stored outside Git with owner-only permissions; branch-scoped Preview server credentials are configured, with billing and native Apple sign-in still disabled pending integration checks. The separate Sign in with Apple key has also been created and securely stored. A private server environment file was prepared outside Git with both keys and a persistent token-encryption key; neither feature has been activated remotely.
...
- Apple credentials and token-encryption material are server-only branch-scoped Preview variables. `APPLE_IAP_ENABLED=false`, `APPLE_SIGN_IN_ENABLED=false`, and `PREVIEW_AUTH_BYPASS=false`. Supabase Apple-provider setup and actual authentication remain pending.
