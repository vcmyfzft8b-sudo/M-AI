...
Open the existing `eu.memoai.memo` record to verify In-App Purchase is enabled. The `.local.nace` record is named for local development. The wrapper now ships one app extension, the Lock Screen recording banner, and it takes the already-registered `eu.memoai.memo.RecordingLiveActivity` identifier rather than adding a record — no Apple-side registration is needed for it. Preserve all existing identifiers.
...
### Lecture recording and the Lock Screen banner

Lectures are recorded natively. `MediaRecorder` inside the web view cannot survive the screen locking — iOS suspends the web content process and ends the microphone track with it — so `ios/MemoAI/LectureRecorder.swift` captures to mono 16 kHz AAC on an `AVAudioSession` the app owns, kept alive by the `audio` background mode. While it runs, a Live Activity drawn by the `RecordingLiveActivity` extension shows the Memo lockup and a running clock on the Lock Screen and in the Dynamic Island. The extension renders the banner; the app starts, updates and ends the activity.

The page drives it over the `memoNative` bridge (`recorderStart`, `recorderPause`, `recorderResume`, `recorderState`, `recorderStop`, `recorderRead`, `recorderDiscard`) and collects the finished m4a in 2 MB slices, since three hours is ~43 MB. Bridge `version` is `2`; `src/lib/mobile/native-recorder.ts` checks it so an installed older build stays on the web recorder. Everything after the hand-off — the limits, the duration, the draft, the upload — is the web app's, unchanged.

Notes on behaviour worth knowing:

- Live Activities can be switched off per app in Settings. Recording still works; `recorderState` reports `bannerAvailable: false`.
- iOS asks the user to confirm the first Live Activity an app posts, on the Lock Screen itself.
