...
Implemented on `codex/ios-app-wrapper`, locally only. The App Store Connect record was created on 15 September 2026: Memo AI, Apple ID `6812409212`, bundle `eu.memoai.memo`, primary language English (U.K.), SKU `memoai-ios`. A fresh signed archive and App Store distribution export include the Apple sign-in entitlement and all four subscription product IDs (`ios/build/MemoAI-signin-trial.xcarchive`, `ios/build/export-signin-trial`). Export verification confirmed the correct app/team, Apple sign-in entitlement, and disabled debugging. The account holder authorized pushing the test branch for its Vercel Preview on 15 September 2026. Production remains unchanged, and no build has been uploaded or submitted for review. Apple billing defaults to disabled. Account setup and the release checks below must finish before enabling it or submitting.

The app supports iPhone and iPad on iOS 17+. It wraps the existing Memo app in WKWebView, opening `https://memoai.eu/auth/continue` to resume a session or show login directly. It adds native Sign in with Apple, StoreKit 2 subscriptions, native file sharing, camera/microphone permission prompts, and connection recovery. There are no native third-party SDK dependencies. The website and server changes in this branch must ship with the wrapper.

### App Store Connect setup in progress

- Created app `6812409212` and subscription group **Memo Premium** (`22387267`). Saved English (U.K.) group display name Memo Premium with app name Memo AI. Monthly is still at level 1 and yearly at level 2: Apple's reorder control rejected the automated drag; both must be placed at level 1 before release.
- Created monthly product `eu.memoai.premium.monthly` (Apple ID `6812410886`, 1 month). Saved €20.00 pricing based on Slovenia, Apple's calculated regional equivalents, availability in all 175 storefronts, and English (U.K.) localization.
- Saved the monthly pay-up-front introductory offer starting 15 September 2026 with no end date in 175 storefronts. The confirmed table shows €9.99 for the first month in Slovenia, Croatia, Bosnia and Herzegovina and Serbia, then €20/month; other regional equivalents vary and are not all exactly 50%. The offer is saved, not tested in Apple Sandbox.
...
- Account holder confirmed Apple prices of **€20/month** and **€129.99/year**; both are saved.
...
- Paid Apps Agreement accepted with the account holder's explicit confirmation. Existing legal entity address retained. The W-8BEN and separate U.S. foreign-status certificate were submitted with the account holder's explicit certifications and both are **Active**. The Paid Apps Agreement has advanced to **Processing**; the new bank account is also processing.
- Revolut EUR payout account added with the account holder's confirmed deposit/reversal authorization and two-factor verification. Apple shows **Processing** and advises up to 24 hours. The bank form rejected `Zgoša 87` as invalid; selected the entered ASCII spelling `Zgosa 87` instead of Apple's suggested `Zgosha 87`. The official business address remains **Zgoša 87, 4275 Begunje na Gorenjskem, Slovenia**. No banking numbers are stored in this repository.
- Created and downloaded the **Memo Billing Server** In-App Purchase key with the account holder's explicit confirmation. Its private key is stored outside Git with owner-only permissions; server environment configuration remains pending. The separate Sign in with Apple key has also been created and securely stored. A private server environment file was prepared outside Git with both keys and a persistent token-encryption key; neither feature has been activated remotely.
- Created monthly trial product `eu.memoai.premium.trial.monthly` (Apple ID `6812525834`) with English (U.K.) metadata, exact €20.00 renewal pricing, availability in all 175 storefronts, and a saved three-day free introductory offer from 15 September 2026 with no end date. The yearly trial product is still outstanding.
...
- Three-day trial presentation and signup-versus-wheel routing: 17 focused catalogue/product/subscription checks passed; TypeScript, changed-file lint and a native Debug simulator build passed (`ios/build/trial-routing-*.log`, `ios/build/trial-native-build.log`). The full web build also passed after these changes (`ios/build/trial-web-build.log`). Seven additional real-component rendering/provider and Apple-auth route checks passed (`ios/build/apple-login-screen-tests.log`), confirming the configured Apple button and email option render on the login screen. Apple trial setup and real lifecycle tests remain outstanding.
- Password sign-in now uses normal Supabase password authentication, with same-origin checks, IP/account rate limits, generic errors, safe redirects, and account-deletion guards. Focused tests, TypeScript, lint and the full web build passed. The phone layout was checked locally; a real synthetic review account still needs to be created and tested.
...
7. Create and validate a dedicated synthetic review account with a strong password. The native email entry page links to `/auth/password`; this uses normal account authentication, not a review bypass. Supply credentials privately in App Store Connect, and verify them in the submitted build. Email OTP remains available. Do not store review passwords in this repository.
8. Select the build and both subscriptions and submit only when all required fields/checks are complete.
