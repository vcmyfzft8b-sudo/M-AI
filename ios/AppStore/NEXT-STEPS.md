...
2. **Web Apple sign-in round trip — verified 17 September 2026:** ten real accounts signed up with Apple on the web during the first day (six with "Hide My Email" relay addresses), all reached onboarding, two signed in again and resumed the same account, and the production logs show no `/auth/callback` errors. Rotate the Supabase client secret before **16 March 2027** (`node scripts/apple/web-client-secret.mjs eu.memoai.web`).
...
4. **Review account — done 18 September 2026:** `apple-review@memoai.eu` exists in production, onboarded, listed in `APPLE_SANDBOX_REVIEW_USER_IDS` and `APP_REVIEW_ACCOUNT_EMAILS`, and signs in with the fixed code in `APP_REVIEW_LOGIN_CODE` (there is no password screen any more; the e-mail code flow is the same as on the web). The code and the account details live in `~/.config/memoai/apple/review-account.env` and go only into App Store Connect's review sign-in fields (user name = the e-mail, password = the code). `ios-device-qa@memoai.eu` (`device-qa-account.env`) is the same kind of account for the account holder's device checks.
