...
// A chunk is generated by whichever request gets the quota reservation first; everyone else
// waits on that generation server-side and, if it is still running when their wait expires,
// gets a 503 telling them to come back. That is a "not ready yet", not a failure — the work is
// still in flight — so the only correct response is to ask again. Nothing did, so the 503
// surfaced as an error and the reader had to press play again by hand.
const TTS_CHUNK_PENDING_CODES = new Set(["tts_generation_pending", "tts_provider_rate_limited"]);
...
const TTS_CHUNK_PENDING_RETRY_BUDGET_MS = 150_000;

function isTtsChunkPendingError(error: unknown) {
  return (
    error instanceof TtsRequestError &&
    (TTS_CHUNK_PENDING_CODES.has(error.code ?? "") || error.status === 429)
  );
}
...
function getTtsChunkRetryDelayMs(attempt: number) {
