...
test("the fallback is clamped to the invocation that remains once the stream has timed out", () => {
  ...
  assert.equal(streamAttemptMs, 180_000, "a healthy stream keeps its full leash");
  ...
  assert.ok(
    fallbackAttemptMs < OPENROUTER_DEFAULT_TIMEOUT_MS,
    "the fallback still asks for 180s it does not have",
  );
  assert.ok(
